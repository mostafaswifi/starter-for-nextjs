  ...
  return (
    <section className="flex w-full flex-col  rounded-xl border border-gray-200 bg-white shadow-sm">
      ...
      <div className="space-y-2 p-6">
        {/* Accordion Item 1 */}
       {
          studentsDataItems?.map(student =>{
            return (<div key={student.$id} className="overflow-hidden rounded-lg border border-gray-200/30 bg-gray-50 max-h-[60px] transition-all duration-300">
          <button
            className="flex w-full items-center justify-between p-4 text-right transition-colors hover:bg-gray-100"
            onClick={(e)=>e.target.parentElement.parentElement.parentElement.classList.toggle('max-h-[500px]')}
          >
            <div className="flex flex-col">
              <span className="text-base font-bold text-gray-900">
                {student?.studentname}
              </span>
              <span className="text-xs text-gray-600">#{student?.reservationnumber}</span>
            </div>
